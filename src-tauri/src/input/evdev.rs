...
#[cfg(target_os = "linux")]
use evdev::{
    uinput::VirtualDeviceBuilder, AttributeSet, EventType, InputEvent, Key, UinputAbsInfo,
};
...
impl EvdevVirtualKeyboard {
    ...
    fn send_key_event(&mut self, key: Key, pressed: bool) -> Result<(), String> {
        let event = InputEvent::new(EventType::KEY, key.code(), if pressed { 1 } else { 0 });
...
mod tests {
    ...
    fn test_evdev_device_creation() {
        match EvdevVirtualKeyboard::new() {
            ...
            Err(e) => println!(
                "Failed to create evdev device (expected without permissions): {}",
                e
            ),
        }
    }
